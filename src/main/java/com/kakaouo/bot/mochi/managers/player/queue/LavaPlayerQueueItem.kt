...
class LavaPlayerQueueItem private constructor(track: AudioTrack, source: CommandSource) :
    AbstractQueueItem(track, source), ILengthAvailableQueue, ILinkAvailableQueue {

    override val title: String get() = audioTrack.info.title
    override val link: String get() = audioTrack.info.uri
    override val length: Long get() = audioTrack.info.length

    companion object {
        suspend fun createQueuesFromQuery(manager: AudioPlayerManager, query: String, source: CommandSource): List<LavaPlayerQueueItem> {
            val list = getTracksFromLavaPlayer(manager, query)
            return list.map { LavaPlayerQueueItem(it, source) }
        }
    }
}